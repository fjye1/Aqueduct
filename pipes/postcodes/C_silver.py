...
from utils.transformations.filters import derive_postcode_sector, london_lsoa_filter
...
def run_pipeline(project_root: Path):
    ...
    # Live postcodes only - DOTERM is only populated once a postcode is retired.
    df = df[df["doterm"].isna()]

    df = london_lsoa_filter(df, lsoa_column="lsoa21cd")
    df = derive_postcode_sector(df, postcode_column="pcds", sector_column="postcode_sector")
