...
from pipes.council_budget.silver.council_budget_silver import flatten_drop
...
config = load_yaml("council_budget.yml")

flatten_drop(
    file_path=config["source"]["file_path"],
    data_row_start=config["transform"]["data_row_start"],
    data_row_end=config["transform"]["data_row_end"],
